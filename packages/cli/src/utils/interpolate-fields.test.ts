...
describe("interpolate-fields", () => {
  ...
  describe("extractFieldNames", () => {
    const check = (template: string, expected: string[]) => {
      expect(extractFieldNames(template)).toEqual(expected);
    };

    it("extracts single field name", () => {
      check("Hello {name}", ["name"]);
    });

    it("extracts multiple field names", () => {
      check("{greeting} {name}", ["greeting", "name"]);
    });

    it("extracts field names with underscores", () => {
      check("{first_name} {last_name}", ["first_name", "last_name"]);
    });

    it("extracts field names with hyphens", () => {
      check("{first-name} {last-name}", ["first-name", "last-name"]);
    });

    it("extracts field names with dots", () => {
      check("{parent.child}", ["parent.child"]);
    });

    it("ignores escaped braces", () => {
      check("\\{literal} {name}", ["name"]);
    });

    it("ignores invalid field names", () => {
      check("{invalid field} {valid}", ["valid"]);
    });

    it("returns empty array for template without fields", () => {
      check("No fields here", []);
    });
  ...
  describe("extractFieldValues", () => {
    ...
    it("round-trips with resolvePath", () => {
      ...
      const resolved = throwIfError(
        resolvePath(mockRecordSchema, navItem, item),
      );
      expect(resolved).toEqual([
        { path: "projects/binder-cli/My Task.md", narrowedEntity: item },
      ]);
      const result = throwIfError(
        extractFieldValues(navItem.path + ".md", resolved[0]!.path),
      );
